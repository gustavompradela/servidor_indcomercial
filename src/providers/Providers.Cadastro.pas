...
type
  ...
  protected
    FCampoChave: String;
    function GetCamposObrigatorios: TArray<TCampoValidacao>; virtual; abstract;
    function GetCamposServidor: TArray<string>; virtual; abstract;
  private
...
class function TCampoValidacao.New(const ANome: string; ATipo: TTipoCampo): TCampoValidacao;
begin
  Result.Nome := ANome;
  Result.Tipo := ATipo;
end;
...
function TProvidersCadastro.Validar(const AJson: TJSONObject; out AErros: TArray<string>): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        LErros.Add(Format('%s é obrigatório', [LCampo.Nome]));
        Continue;
      end;
      ...
      case LCampo.Tipo of
        tcTexto:
          if not (LValue is TJSONString) then
            LErros.Add(Format('%s deve ser texto', [LCampo.Nome]))
          else if TJSONString(LValue).Value.Trim.IsEmpty then
            LErros.Add(Format('%s não pode estar em branco', [LCampo.Nome]));

        tcInteiro:
          if not TryStrToInt(LValue.Value, LInt) then
            LErros.Add(Format('%s deve ser um número inteiro', [LCampo.Nome]));

        tcNumero:
          if not TryStrToFloat(LValue.Value, LNum) then
            LErros.Add(Format('%s deve ser numérico', [LCampo.Nome]));
      end;
    end;
    AErros := LErros.ToArray;
